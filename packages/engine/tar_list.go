...
func (r *Runner) runListTar(ctx context.Context, opts cli.Options, reporter *archiveprogress.Reporter, ref locator.Ref, ar io.ReadCloser, info archiveReaderInfo, excludeMatcher *archivepath.CompiledPathMatcher) (int, error) {
	...
	scan := func(scanReader io.ReadCloser, scanInfo archiveReaderInfo) (int, error) {
		return r.scanTarArchiveFromReader(ctx, opts, reporter, scanInfo, archiveutil.NameHint(ref), scanReader, func(hdr *tar.Header, tr *tar.Reader) (int, error) {
			if shouldSkipReadMember(memberMatcher, excludeMatcher, hdr.Name) {
