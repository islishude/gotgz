...
func (r *Runner) runListZip(ctx context.Context, opts cli.Options, reporter *archiveprogress.Reporter, archiveRef locator.Ref, ar io.ReadCloser, info archiveReaderInfo, excludeMatcher *archivepath.CompiledPathMatcher) (int, error) {
	...
	if len(volumes) == 1 {
		...
		zipWarnings, err := r.withZipReader(ctx, archiveRef, ar, info, reporter, func(zr *zip.Reader) (int, error) {
			return r.listZipReader(ctx, zr, memberMatcher, excludeMatcher, reporter)
		})
...
func (r *Runner) listZipReader(ctx context.Context, zr *zip.Reader, memberMatcher, excludeMatcher *archivepath.CompiledPathMatcher, reporter *archiveprogress.Reporter) (int, error) {
